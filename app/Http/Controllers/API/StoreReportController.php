...
class StoreReportController extends Controller
{
	...
	public function stockExpiryReport(Request $request)
	{
		...
		if($supplier>0){
			$expirySQL=	"SELECT psr.id,pr.receipt_no,pr.bill_no,psr.sub_total, p.account_title, psr.item_name, (psr.expiry_date), psr.batch_no, (pr.receipt_date), s.qty as total_unit, psr.purchase_price, psr.purchase_disc, psr.tax_1, psr.tax_2, psr.tax_3 FROM `pos_sub_receipts` psr, pos_receipts pr, profilers p, stocks s WHERE psr.expiry_date between '$date1' and '$date2' and pr.bill_no!='' and pr.id=psr.pos_receipt_id and pr.profile_id=p.id and pr.profile_id=$supplier and psr.stock_id=s.id order by psr.expiry_date DESC;";

		} else{
			$expirySQL=		"SELECT psr.id,pr.receipt_no,pr.bill_no,psr.sub_total, p.account_title, psr.item_name, (psr.expiry_date), psr.batch_no,(pr.receipt_date), s.qty as total_unit, psr.purchase_price, psr.purchase_disc, psr.tax_1, psr.tax_2, psr.tax_3 FROM `pos_sub_receipts` psr, pos_receipts pr, profilers p, stocks s WHERE psr.expiry_date between '$date1' and '$date2' and pr.bill_no!='' and pr.id=psr.pos_receipt_id and pr.profile_id=p.id and psr.stock_id=s.id order by psr.expiry_date DESC;";
