                    ...
                    <div class="p-col">
                        <div class="p-field">
                            <label
                                ...
                                :class="{
                                    'p-error':
                                        v$.receiptDueDate.$invalid && submitted
                                }"
                                >Due Date</label
                            >
                            <Calendar
                                id="receiptDueDate"
                                v-model="v$.receiptDueDate.$model"
                                :class="{
                                    'p-invalid':
                                        v$.receiptDueDate.$invalid && submitted
                                }"
                                ...
                                dateFormat="dd-mm-yy"
                            />
                            <span v-if="v$.receiptDueDate.$error && submitted">
                                <span
                                    id="p-error"
                                    v-for="(error, index) of v$.receiptDueDate
                                        .$errors"
                                    :key="index"
                                >
                                    <small class="p-error">{{
                            ...
                            <small
                                v-else-if="
                                    (v$.receiptDueDate.$invalid && submitted) ||
                                        v$.receiptDueDate.$pending.$response
                                "
                                class="p-error"
                                >{{
                                    v$.receiptDueDate.required.$message.replace(
                                        "Value",
                                        "Due Date"
                                    )
                                }}</small
                    ...
                    <div class="p-col-12">
                        <table class="table table-bordered total-lables">
                            <tr>
                                ...
                                <td>Balance :  {{currency}} {{ formatAmount(netTotal) }}</td>
                            </tr>
                        </table>
                    </div>
                </div>
...
export default class Invoice extends mixins(UtilityOptions) {
    ...
    saveItem(isFormValid) {
        ...
        if (isFormValid && this.validateHeadList.length == 0) {
            ...
            if (this.item.id != 0) {
                ...
                this.state.receiptDueDate = moment(
    ...
    openReceivePaymentDialog(data) {
        ...
        this.dialogTitle = "Receive Invoice Payment";
